...
export default function FirewallPanel() {
  ...
  const load = () => {
    api.firewallProviders().then((d) => setProviders(d.providers));
    api.blockedIps().then((d) => setBlocked(d.blocked_ips));
    if (hasPermission('quarantine:manage')) api.listQuarantine().then((d) => setQuarantine(d.quarantine));
    if (hasPermission('approvals:manage')) api.listPendingApprovals().then((d) => setApprovals(d.approvals));
  };
  ...
  const block = async (e: FormEvent) => {
    e.preventDefault();
    if (!hasPermission('ip:block') || !justification.trim()) {
      setMsg('يتطلب تبريراً أمنياً للعملية');
      return;
    }
    try {
  ...
  const release = async (qid: string) => {
    if (!confirmPassword.trim() || !justification.trim()) {
      alert('يتطلب تأكيد كلمة المرور والتبرير الأمني');
      return;
    }
  ...
  return (
    <div>
      <div className="cds-page-hero mb-4">
        ...
        <p>إدارة الحظر، العزل، والموافقات المعلقة</p>
      </div>
          ...
          {hasPermission('ip:block') && (
            <div className="cds-card mb-3">
              ...
              <form className="cds-card-body" onSubmit={block}>
                ...
                <div className="mb-2">
                  <label className="form-label">التبرير الأمني *</label>
                  <textarea className="form-control" value={justification} onChange={(e) => setJustification(e.target.value)} required />
                </div>
                <div className="mb-2">
                  <label className="form-label">تأكيد كلمة المرور</label>
                  <input type="password" className="form-control" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
                </div>
                <button type="submit" className="btn btn-danger">تنفيذ الحظر</button>
              </form>
            </div>
          )}
